...
import { Tabs, Tab, Button, Input } from 'react-bootstrap';

// Import constants to use as a starting point for the user's .gulpfile and package.json
...
export default class App extends Component {
  constructor(props) {
    ...
    this.accordionSection = this.accordionSection.bind(this);
  ...
  // Hitting download button sends new gulpfile content to gulpfile and zips all three files to
  // download for client
  download(event) {
  ...
  search(event) {
    ...
    $.get(`http://npmsearch.com/query?fields=name,keywords,rating,description,version&q=name:${event.target.value}&sort=rating:desc`, (data) => {
      ...
      data.forEach((pkg) => {
        ...
        if (pkgName.findIndex((el) => el === pkg.name[0]) === -1) {
  ...
  addToPackageJson() {
    ...
    $.get(`http://npmsearch.com/query?fields=name,keywords,rating,description,version&q=name:${this.state.npmSearch}`, (data) => {
      ...
      const selectedFramework = this.state.currentJsonFramework;

      newPackages += `,\n\t\t\t\t"${this.state.npmSearch}": "^${pkgVersion}"`;
      this.setState({
        json: selectedFramework.start + newPackages + selectedFramework.end,
        jsonDependencies: newPackages,
      });
    });
  }
  ...
  addToGulpfile() {
    const gulpSearch = this.state.npmSearch;
    let plugins = this.state.gulpPlugins;
    const selectedFramework = this.state.currentGulpFramework;
  ...
  saveUser(event) {
    ...
    $.ajax({
      type: 'POST',
      url: '/register',
      data, // Whatever is in data will become req.body.
      contentType: 'application/json',
    })
    .done(() => {
      that.setState({
        loggedIn: true,
        registerErrorMessages:
        `Registration successful. You are now logged in as ${that.state.username}.`,
        password: '',
      });
    })
    .fail(() => {
  ...
  // Shows the appropriate accordion section
  accordionSection(event) {
    event.preventDefault();

    const toAccordion = event.target.value;
    const accordionObj = this.state.accordionIsOpen;

    // First close all sections
    for (const section in accordionObj) {
  ...
  // Uses button's dispatchMarker to determine the framework that was clicked on and uses helper
  // func setEditorDefaults to update state.
  // Note: We can't pass down setEditorDefaults directly -this  results in calling this.setState
  // in infinite loop.
      ...
      <div id="App">
        ...
        <div className="row">
          ...
          <div className="col-md-2 rightContainer">
            <Input type="text" onChange={this.saveProjectName}
        ...
        <div className="row">
          <Gulpoptions
            ...
            accordionSection={this.accordionSection}
          ...
          <div className="col-md-7">
            ...
            <Tabs defaultActiveKey={1}>
              <Tab eventKey={1} title="Gulpfile">
                <form className="npm-search">
                  <div className="col-md-10">
                    <Input type="search" list="packages" placeholder="Search Gulp Plugins"
              ...
              <Tab eventKey={2} title="package.json">
                <form className="npm-search">
                  <div className="col-md-10">
                    <Input type="search" list="packages" placeholder="Search NPM Packages"
                      onChange={this.search}
                    />
                    <datalist id="packages">{npmResults}</datalist>
                  </div>
                  <div className="col-md-2">
                    <Button onClick={this.addToPackageJson} className="right">
                      + package.json
                    </Button>
                  </div>
                  <br style={{ clear: 'both' }} />
